...
function submitRequest(){
  ...
  addNewRequest(newId, {
    ...
    destinationAddress: deliveryAddress, // Stores the delivery destination
  ...
  pushTrail(newId, `${newId} submitted for ${ids.length} item${ids.length>1?'s':''} (Recipient: ${recipientName} · Destination: ${deliveryAddress})${pref?` — packaging preference: ${pref}`:''} — awaiting Admin approval`, false);

  toast(`${newId} submitted — pending Admin approval`);
...
function taskSubIdx(r){
  if(r.stageIdx < IDX.COLLECTION) return -1;
...
function renderTasks(){
  ...
  container.innerHTML = tasks.map(r => {
    const sub = taskSubIdx(r);
    const catRule = classCategories.find(c => c.name === r.category);

    return `
    <div class="task-card ${r.halted ? 'halted' : ''}" style="background: var(--cream); border: 1px solid var(--line); border-radius: 16px; padding: 20px; margin-bottom: 16px; display: flex; flex-direction: column; gap: 16px;">
      
      <!-- Top Row: Header & Status -->
      <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; border-bottom: 1px dashed var(--line); padding-bottom: 12px;">
        <div>
          <div style="display: flex; align-items: center; gap: 10px;">
            <strong style="font-size: 18px; font-weight: 800; color: var(--ink);">${r.id}</strong>
            <span class="pill ${r.halted ? 'halted' : 'approved'}" style="font-size: 11px;">${r.halted ? 'Halted' : 'Active'}</span>
          </div>
          <div style="font-size: 13px; color: var(--ink-soft); margin-top: 4px;">
            <strong>Requester:</strong> ${escapeHtml(r.requester)} · ${r.itemsCount} item${r.itemsCount > 1 ? 's' : ''} (${r.unitsCount} units)
          </div>
        </div>

        <!-- Tags / Rule Badges -->
        <div style="display: flex; flex-wrap: wrap; gap: 6px; justify-content: flex-end;">
          ${catRule ? `<span class="pill" style="background: rgba(148, 166, 132, 0.2); color: var(--sage-dark); font-size: 11px;">🏷️ ${r.category}: ${catRule.rule}</span>` : ''}
          ${r.packaging ? `<span class="pill" style="background: var(--cream-2); color: var(--ink); font-size: 11px;">📦 ${r.packaging}</span>` : ''}
        </div>
      </div>

      <!-- Stage Step Pipeline Tracker -->
      <div>
        <label class="form-label" style="margin-bottom: 8px;">Pipeline Progress</label>
        <div class="stage-track" style="display: flex; gap: 8px; flex-wrap: wrap;">
          ${HANDLER_SUBSTAGES.map((s, idx) => `
            <span class="stage-chip ${idx < sub ? 'done' : idx === sub ? 'active' : ''}" 
                  style="flex: 1; min-width: 130px; text-align: center; padding: 8px 10px; border-radius: 8px; font-size: 12px; font-weight: 600; border: 1px solid var(--line); background: ${idx < sub ? 'rgba(148, 166, 132, 0.25)' : idx === sub ? 'var(--sage)' : 'var(--cream-2)'}; color: ${idx === sub ? '#12261B' : 'var(--ink)'};">
              ${idx + 1}. ${s}
            </span>
          `).join('')}
        </div>
      ...
      ${r.halted ? `
        <div class="halted-banner" style="padding: 12px; background: rgba(196, 122, 87, 0.15); border: 1px solid var(--danger); border-radius: 10px; display: flex; align-items: center; justify-content: space-between;">
          <span style="font-size: 13px; color: var(--danger); font-weight: 600;">This task is halted and paused for review.${r.haltReason ? (' Reason: ' + escapeHtml(r.haltReason)) : ''}</span>
          <button class="btn btn-sage btn-sm" onclick="resumeTask('${r.id}')">Resume Task</button>
        </div>
      ` : `
        <!-- Bottom Action Bar -->
        <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 12px; background: var(--cream-2); padding: 12px 14px; border-radius: 12px; border: 1px solid var(--line);">
          ...
          <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap; flex: 1;">
            <select class="pack-select" onchange="setPackaging('${r.id}', this.value)" style="padding: 8px 12px; border: 1px solid var(--line); border-radius: 8px; background: var(--cream); font-size: 12.5px; color: var(--ink);">
              <option value="">Packaging preference…</option>
            ...
            <button class="btn btn-sage btn-sm" onclick="advanceStage('${r.id}')">Advance Stage →</button>
          </div>

          <!-- Secondary Tools & Halt Group -->
          <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
            <button class="btn btn-ghost btn-sm" onclick="toggleQc('${r.id}')">QC</button>
            <button class="btn btn-ghost btn-sm" onclick="togglePhotos('${r.id}')">Photos</button>
            <button class="btn btn-ghost btn-sm" onclick="toggleCourier('${r.id}')">Courier</button>
            <button class="btn btn-outline btn-sm" style="color: var(--danger); border-color: var(--danger);" onclick="haltTask('${r.id}')">Halt</button>
          </div>
        </div>

        <!-- Collapsible Tool Panels -->
        <div class="qc-box" id="qc-${r.id}" style="display:none; padding: 12px; background: var(--cream-2); border-radius: 10px; margin-top: 8px;">
          <div style="font-size:12px;font-weight:700;color:var(--ink-soft); margin-bottom: 8px;">
            Initial Quality Inspection
            ${r.qcStatus === 'passed' ? ' — <span style="color:var(--sage-dark);">Passed ✓</span>' : r.qcStatus === 'failed' ? ' — <span style="color:var(--danger);">Failed</span>' : ''}
          </div>
          <div class="qc-actions" style="display: flex; gap: 8px;">
            <button class="btn btn-sage btn-sm" onclick="qcResult('${r.id}', true)">Pass — Proceed to Packing</button>
            <button class="btn btn-outline btn-sm" onclick="qcResult('${r.id}', false)">Fail — Mark Damaged</button>
          </div>
          <div class="qc-fail-note" id="qc-fail-${r.id}" style="display:none; margin-top: 10px;">
            <label class="form-label">Damage Remarks (required)</label>
            <textarea id="qc-remarks-${r.id}" class="vt-input" placeholder="Describe the damage found during inspection…" style="width: 100%; min-height: 70px; padding: 8px; border-radius: 8px; font-size: 12px;"></textarea>
            <button class="btn btn-terracotta btn-sm" style="margin-top:8px;" onclick="submitDamageReport('${r.id}')">Submit &amp; Notify Admin</button>
          </div>
        </div>

        <div class="photo-box" id="photos-${r.id}" style="display:none; padding: 12px; background: var(--cream-2); border-radius: 10px; margin-top: 8px;">
          <div style="font-size:12px;font-weight:700;color:var(--ink-soft); margin-bottom: 4px;">Condition Photo Documentation</div>
          <div class="section-hint" style="margin-bottom: 8px;">Take a photo with your camera, or upload existing image files.</div>
          <div class="photo-slots" style="display: flex; gap: 8px;">
            <label class="photo-slot" style="cursor:pointer; padding: 6px 12px; background: var(--cream); border: 1px solid var(--line); border-radius: 8px; font-size: 12px;">📷 Camera
              <input type="file" accept="image/*" capture="environment" style="display:none;" onchange="fillPhotoInput(this,'${r.id}')">
            </label>
            <label class="photo-slot" style="cursor:pointer; padding: 6px 12px; background: var(--cream); border: 1px solid var(--line); border-radius: 8px; font-size: 12px;">＋ Upload
              <input type="file" accept="image/*" multiple style="display:none;" onchange="fillPhotoInput(this,'${r.id}')">
            </label>
          </div>
          <div class="photo-slots" id="photo-preview-${r.id}" style="margin-top: 8px; display: flex; gap: 6px;"></div>
        </div>

        <div class="courier-box" id="courier-${r.id}" style="display:none; padding: 12px; background: var(--cream-2); border-radius: 10px; margin-top: 8px;">
          <div class="track-toggle" style="margin-bottom:12px; display: flex; gap: 6px;">
            <button class="btn btn-sm ${r.courier.type === 'internal' ? 'btn-sage' : 'btn-ghost'}" onclick="setCourierType('${r.id}','internal')">Internal Messenger</button>
            <button class="btn btn-sm ${r.courier.type === 'third-party' ? 'btn-sage' : 'btn-ghost'}" onclick="setCourierType('${r.id}','third-party')">Third-Party Courier</button>
          </div>
          ${r.courier.type === 'internal' ? `
            <div class="section-hint">Internal Messenger pickup uses live GPS tracking once the Messenger starts delivery from their own queue.</div>
            <button class="btn btn-terracotta btn-sm" style="margin-top:10px;" ${sub < 3 ? 'disabled' : ''} onclick="confirmInternalHandoff('${r.id}')">Confirm Ready for Messenger Pickup</button>
          ` : `
            ...
            <div class="section-hint" style="margin-top: 6px;">Third-party couriers use static milestone tracking — no live GPS.</div>
            <button class="btn btn-terracotta btn-sm" style="margin-top:10px;" ${sub < 3 ? 'disabled' : ''} onclick="saveCourier('${r.id}')">Save &amp; Dispatch</button>
          `}
        </div>
      `}
    </div>`;
...
function refreshTaskStats(){
  ...
  if(!elAwaiting) return; // Prevents crash on admin.html
  ...
  if(document.getElementById('statDone')) document.getElementById('statDone').textContent = handlerCompletedToday;
}
...
function advanceStage(id){
  ...
  if(r.stageIdx === IDX.APPROVED){
    ...
    r.stageIdx = IDX.QC;
    pushTrail(id, `${id} — moved to Quality Control`);
    toast(`${id} moved to Quality Control`);
  } else if(r.stageIdx === IDX.QC){
    if(r.qcStatus!=='passed'){ toast('Run Quality Control and Pass it before proceeding to Packing'); return; }
    r.stageIdx = IDX.PACKING;
    pushTrail(id, `${id} — moved to Packing`);
    toast(`${id} moved to Packing`);
  } else if(r.stageIdx === IDX.PACKING){
    if(!r.packaging){ toast('Select a packaging preference before marking Ready to Ship'); return; }
    r.stageIdx = IDX.READY;
    pushTrail(id, `${id} packed with ${r.packaging} — Ready to Ship`);
    toast(`${id} is now Ready to Ship`);
    notifyConvo('req', 'c1', `Your request ${id} is packed and ready to ship.`);
  } else {
    toast('Use Courier Assignment below to dispatch this request');
    return;
  }
  renderTasks();
  refreshTaskStats();
...
function haltTask(id){
  ...
  notifyConvo('admin', 'ac2', `${id} has been halted by the Handler and needs review.`);
  renderTasks();
  refreshTaskStats();
  renderHandlerTrailBoxes();
  renderAdminRequests();
...
function qcResult(id, passed){
  const r = REQUESTS[id];
  if(passed){
    r.qcStatus = 'passed';
    if(r.stageIdx < IDX.PACKING) r.stageIdx = IDX.PACKING;
    pushTrail(id, `Quality Control passed for ${id} — proceeding to Packing`);
    toast(`${id} passed QC`);
    notifyConvo('req', 'c1', `Quality Control passed for ${id}.`);
    renderTasks();
    refreshTaskStats();
    renderHandlerTrailBoxes();
    renderOutbound();
    ...
    document.getElementById('qc-fail-'+id).classList.add('show');
  }
}
...
function fillPhotoInput(inputEl, id){
  ...
  toast(`${names.length} photo${names.length>1?'s':''} attached to ${id}`);
}
...
function confirmInternalHandoff(id){
  const r = REQUESTS[id];
  r.courier.type = 'internal';
  pushTrail(id, `${id} confirmed Ready to Ship — awaiting internal Messenger pickup`);
  toast(`${id} is ready — the Messenger will now see it in their queue`);
  notifyConvo('messenger', 'mc2', `${id} is packed and ready at the loading dock.`);
  notifyConvo('req', 'c2', `Your request ${id} is ready to ship.`);
  renderTasks();
  refreshTaskStats();
  renderHandlerTrailBoxes();
  if(document.getElementById('messengerTaskList')) renderMessengerDeliveries();
}

function saveCourier(id){
  const name = document.getElementById('courier-name-'+id).value.trim();
  const wb = document.getElementById('courier-wb-'+id).value.trim();
  if(!name || !wb){ toast('Enter both courier name and waybill number'); return; }
  const r = REQUESTS[id];
  ...
  pushTrail(id, `${id} dispatched via ${name} (Waybill ${wb}) — Third-Party Courier, static tracking`);
  toast(`${id} dispatched via ${name} — removed from Task Queue`);
...
/* ============================================================
   MESSENGER — Delivery Task Queue
   ============================================================ */
function refreshMessengerStats(){
  const elQueued = document.getElementById('mStatQueued');
  if(!elQueued) return; // Prevents crash on admin.html

  const tasks = reqList().filter(r=>r.courier.type==='internal' && !r.halted && (r.stageIdx===IDX.READY || r.stageIdx===IDX.TRANSIT));
  elQueued.textContent = tasks.filter(r=>r.stageIdx===IDX.READY).length;
  if(document.getElementById('mStatTransit')) document.getElementById('mStatTransit').textContent = tasks.filter(r=>r.stageIdx===IDX.TRANSIT).length;
  if(document.getElementById('mStatDone')) document.getElementById('mStatDone').textContent = messengerCompletedToday;
}
...
function renderMessengerDeliveries(){
  ...
  if(tasks.length===0){
    container.innerHTML = `<div class="empty-state">No deliveries queued right now.</div>`;
    return;
  }
  container.innerHTML = tasks.map(r=>{
    const progress = r.transitProgress||0;
    const status = r.stageIdx===IDX.READY ? 'queued' : (progress>=100 ? 'arrived' : 'transit');
    const statusPill = status==='queued' ? `<span class="pill submitted">Awaiting Pickup</span>`
      : status==='transit' ? `<span class="pill transit">In Transit — ${progress}%</span>`
      : `<span class="pill qc">Arrived — Pending Proof of Delivery</span>`;
    ...
    <div class="delivery-card">
      <div class="delivery-top">
        <div>
          <div class="task-id">${r.id}</div>
          <div class="task-req">From: ${r.requester} · To: ${r.receiver||'Unassigned'} · ${r.itemsCount} items · ${r.unitsCount} units</div>
        </div>
        ${statusPill}
      </div>
      ...
      ${status==='queued' ? `
        <div class="task-actions">
          <button class="btn btn-terracotta btn-sm" onclick="startDelivery('${r.id}')">Start Delivery</button>
        </div>
      ` : ''}

      ${status==='transit' ? `
        <div class="track-route" style="margin:20px 6px;">
          <span class="endpoint start">Warehouse</span>
          <span class="endpoint end">Destination</span>
          <div class="fill" id="mFill-${r.id}" style="width:${progress}%;"></div>
          <div class="marker" id="mMarker-${r.id}" style="left:${progress}%;"></div>
        </div>
        <div class="gps-note">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s7-6.1 7-11a7 7 0 1 0-14 0c0 4.9 7 11 7 11z"/><circle cx="12" cy="10" r="2.4"/></svg>
          Live GPS coordinates recording every 10–15 seconds · visible to Admin, Requester &amp; Receiver
        </div>
        <div class="task-actions">
          <button class="btn btn-ghost btn-sm" onclick="toast('Transit update sent to Requester and Receiver')">Send Transit Update</button>
        </div>
      ` : ''}

      ${status==='arrived' ? `
        <div class="task-actions">
          <button class="btn btn-sage btn-sm" onclick="togglePod('${r.id}')">Proof of Delivery</button>
        </div>
        <div class="pod-box" id="mPod-${r.id}">
          <div style="font-size:12px;font-weight:700;color:var(--ink-soft);">Photo of the package at drop-off</div>
          <div class="photo-slots">
            <label class="photo-slot" style="cursor:pointer;">📷 Camera
              <input type="file" accept="image/*" capture="environment" style="display:none;" onchange="podFillPhoto(this,'${r.id}')">
            </label>
            <label class="photo-slot" style="cursor:pointer;">＋ Upload
              <input type="file" accept="image/*" style="display:none;" onchange="podFillPhoto(this,'${r.id}')">
            </label>
          </div>
          <div class="photo-slots" id="pod-preview-${r.id}"></div>
          <button class="btn btn-terracotta btn-sm" style="margin-top:12px;" onclick="confirmDropoff('${r.id}')">Confirm Drop-off</button>
        </div>
      ` : ''}
    </div>`;
  }).join('');
...
function podFillPhoto(inputEl, id){
  const files = inputEl.files;
  if(!files || files.length===0) return;
  ...
  chip.textContent = '✓ ' + (names.length>1 ? `${names.length} files` : names[0]);
  preview.appendChild(chip);
  toast(`${names.length} photo${names.length>1?'s':''} attached`);
}
...
function startDelivery(id){
  ...
  if(!r) return;
  r.stageIdx = IDX.TRANSIT;
  r.transitProgress = 10;
  pushTrail(id, `${id} marked In Transit — GPS tracking active`);
  toast(`${id} — Start Delivery clicked, GPS tracking active`);
  notifyConvo('receiver', 'rc2', `On the way with your package, ${id}.`);
  ...
  messengerTrackingIntervals[id] = setInterval(()=>{
    r.transitProgress = Math.min(100, r.transitProgress + (6 + Math.floor(Math.random()*8)));
    if(r.transitProgress>=100){
      clearInterval(messengerTrackingIntervals[id]);
      delete messengerTrackingIntervals[id];
      r.stageIdx = IDX.ARRIVED;
      pushTrail(id, `${id} arrived — GPS tracking stopped`);
      toast(`${id} arrived — GPS tracking stopped, awaiting Proof of Delivery`);
      renderOutbound();
      renderReqTrailBoxes();
      if(document.getElementById('receiverInboundBody')) renderReceiverInbound();
    }
...
function togglePod(id){ document.getElementById('mPod-'+id).classList.toggle('show'); }

function confirmDropoff(id){
  const r = REQUESTS[id];
  if(!r) return;
  r.stageIdx = IDX.INSPECTION;
  messengerCompletedToday++;
  pushTrail(id, `${id} dropped off — pending Receiver Final Inspection`);
  toast(`${id} dropped off — Receiver notified for Final Inspection`);
  notifyConvo('receiver', 'rc2', `${id} has arrived — please proceed to Final Inspection.`);
  notifyConvo('req', 'c3', `Receiver confirms ${id} has arrived.`);
  renderMessengerDeliveries();
  refreshMessengerStats();
  renderMessengerHistory();
  renderOutbound();
  renderReqTrailBoxes();
  if(document.getElementById('receiverInboundBody')){ renderReceiverInbound(); populateInspectSelect(); }
}

function renderMessengerHistory(){
  const rows = reqList().filter(r=>r.courier.type==='internal' && r.stageIdx>=IDX.INSPECTION);
  document.getElementById('messengerHistoryBody').innerHTML = rows.length ? rows.map(r=>{
    const status = r.disputeId ? 'Delivered — Dispute Raised' : (r.stageIdx>=IDX.COMPLETED ? 'Delivered — Accepted' : 'Delivered — Pending Inspection');
    return `
    <tr>
      <td><strong>${r.id}</strong></td>
      <td>${r.requester}</td>
      <td>${r.receiver||'—'}</td>
      <td>${r.dateCreated}</td>
      <td><span class="pill ${status.includes('Dispute')?'rejected':'complete'}">${status}</span></td>
    </tr>`;
  }).join('') : `<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:20px;">No delivery history yet.</td></tr>`;
}
...
function renderReceiverInbound(){
  const rows = reqList().filter(r=>r.receiver===IDENTITY.receiver && !r.halted && !r.disputeId && r.stageIdx>=IDX.READY && r.stageIdx<=IDX.ARRIVED);
  const container = document.getElementById('receiverInboundBody');
  if(rows.length===0){
    container.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:20px;">No incoming deliveries right now.</td></tr>`;
    return;
  }
  container.innerHTML = rows.map(r=>{
    ...
    if(r.stageIdx===IDX.READY){
      ...
      if(r.courier.type==='third-party'){
        ...
        actionBtn = `<button class="btn btn-outline btn-sm" onclick="markArrived('${r.id}')">Mark Arrived</button>`;
      } else {
      ...
      actionBtn = `<button class="btn btn-sage btn-sm" onclick="goInspect('${r.id}')">Inspect</button>`;
    }
    return `
    <tr>
      <td><strong>${r.id}</strong></td>
...
function populateInspectSelect(){
  ...
  const inspectable = reqList().filter(r=>r.receiver===IDENTITY.receiver && !r.halted && !r.disputeId && (r.stageIdx===IDX.ARRIVED || r.stageIdx===IDX.INSPECTION));
  if(inspectable.length===0){
    sel.innerHTML = `<option>No deliveries ready for inspection</option>`;
    document.getElementById('inspectionBody').innerHTML = `<div class="empty-state">Nothing has arrived yet. Check Incoming Deliveries.</div>`;
    return;
  }
  sel.innerHTML = inspectable.map(r=>`<option value="${r.id}">${r.id}</option>`).join('');
...
function renderWizardStep(id){
  ...
  if(r.waiver && inspectionWizardStep<2){
    wrap.innerHTML = `
      <div class="waiver-note">
        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 12l2 2 4-4"/><circle cx="12" cy="12" r="9"/></svg>
        An electronic waiver is on file for this item — condition check and functional testing are skipped once acknowledged.
      </div>
      <button class="btn btn-terracotta btn-sm" style="margin-top:14px;" onclick="acknowledgeWaiver('${id}')">Acknowledge Waiver &amp; Skip Testing</button>
    `;
    return;
  }
  ...
  if(inspectionWizardStep===0){
    wrap.innerHTML = `
      ...
      <div class="section-hint">Compare the package's current condition against the initial condition photos taken during Quality Control.</div>
      <div class="task-actions">
    ...
    wrap.innerHTML = `
      ...
      <div class="section-hint">Condition check and functional testing have passed. You may now accept the delivery.</div>
      <div class="task-actions">
        <button class="btn btn-terracotta btn-sm" onclick="completeDelivery('${id}')">Complete Delivery</button>
...
function acknowledgeWaiver(id){
  inspectionWizardStep = 2;
  renderWizardSteps();
  toast(`Waiver acknowledged for ${id} — testing skipped`);
  renderWizardStep(id);
}
...
function renderReceiverTrail(){
  const rows = reqList().filter(r=>r.receiver===IDENTITY.receiver && (r.stageIdx>=IDX.ARRIVED || r.disputeId));
  const list = document.getElementById('receiverTrailList');
  list.innerHTML = rows.length ? rows.map(r=>
    r.trail.map(t=>`<li class="${t.done?'done':''}"><div class="t-head"><span>${r.id} — ${t.t}</span></div><div class="t-date">${t.d}</div></li>`).join('')
  ).join('') : `<li><div class="t-head"><span>No activity yet.</span></div></li>`;
}
...
function renderReceiverDisputesList(){
  ...
  if(!container) return;
  const rows = Object.values(adminDisputes).filter(d=>d.raisedBy.includes(IDENTITY.receiver));
  container.innerHTML = rows.length ? rows.map(d=>`
    <div class="delivery-card">
      <div class="delivery-top">
        <div>
          <div class="task-id">${d.id} — ${d.reqId}</div>
          <div class="task-req">${d.date} · ${d.desc}</div>
        </div>
        <span class="pill ${d.status==='Open'?'submitted':'complete'}">${d.status}</span>
      </div>
    </div>
  `).join('') : `<div class="empty-state">You haven't raised any disputes.</div>`;
  const badge = document.getElementById('receiverDisputeBadge');
  if(badge){ const n = rows.filter(d=>d.status==='Open').length; badge.textContent = n; badge.style.display = n?'inline-block':'none'; }
}
...
function toast(message) {
  ...
  if (window.toastTimeout) clearTimeout(window.toastTimer);
